...
def make_cube_dualseq(size=1.0) -> DualSeq:
    cmds = [
        "COOR", "FACE", "LOOP",
        "LINE", "LINE", "LINE", "LINE",
        "EXTRUDE_NEW"
    ...
    return DualSeq(cmds=cmds, args=args, uid="cube")

def make_cylinder_dualseq(r=0.5, h=1.0) -> DualSeq:
    cmds = [
        "COOR", "FACE", "LOOP",
        "CIRCLE",
        "EXTRUDE_NEW"
    ...
    return DualSeq(cmds=cmds, args=args, uid="cylinder")

def make_cylinder_on_top_dualseq(cube_size=1.0, cyl_r=0.3, cyl_h=0.5) -> DualSeq:
    cmds = [
        "COOR", "FACE", "LOOP",
        "LINE", "LINE", "LINE", "LINE",
        "EXTRUDE_NEW",
    ...
    return DualSeq(cmds=cmds, args=args, uid="cylinder_on_top")

def visualize_basic_shapes_with_representations(out_dir="out/visualiations"):
    from utils.render import render_dual_seq_with_representations_to_img
    from utils.representations.dual_seq.dual_seq import DualSeqMetadata
    
    cube_ds = make_cube_dualseq()
    cyl_ds = make_cylinder_dualseq()
    cot_ds = make_cylinder_on_top_dualseq()
    
    metadata = DualSeqMetadata()
    metadata.fit([cube_ds, cyl_ds, cot_ds])
    
    os.makedirs(out_dir, exist_ok=True)
    
    render_dual_seq_with_representations_to_img(cube_ds, os.path.join(out_dir, "cube_representations.png"), metadata)
    render_dual_seq_with_representations_to_img(cyl_ds, os.path.join(out_dir, "cylinder_representations.png"), metadata)
    render_dual_seq_with_representations_to_img(cot_ds, os.path.join(out_dir, "cylinder_on_top_representations.png"), metadata)
    
    print(f"Rendered basic shapes with representations to {out_dir}")