...
class TokenizedArgsWrapper(torch.nn.Module):
    """A wrapper for the TokenizedArgsBaseModel for inference and training"""
        ...
        for k, v in state_dict.items():
            if k in model_state_dict:
                if v.shape == model_state_dict[k].shape:
                    ...
                    print(f"Shape mismatch for key {k}: checkpoint shape {v.shape}, model shape {model_state_dict[k].shape}. Skipping this key.")
            else:
