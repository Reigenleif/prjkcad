...
class T5EncDualDecCAD(nn.Module):
    """T5 encoder + dual-stream decoder for text-to-DualSeq training."""
    ...
    def __init__(
        ...
        side_encoder_layer = nn.TransformerEncoderLayer(
            d_model=d_model,
            nhead=n_heads,
            batch_first=True,
        )
        ...
        self.side_encoder = nn.TransformerEncoder(side_encoder_layer, num_layers=n_layers)
    ...
    def forward(
        ...
        if side_input_ids is None:
            side_input_ids = decoder_input_ids
        if side_input_args is None:
            side_input_args = decoder_input_args
        ...
        side_state = self.dec_embedding(side_input_ids) + self.arg_embedding(side_input_args) + pos_embed

        side_padding_mask = decoder_attention_mask == 0
        side_out = self.side_encoder(
            side_state,
            src_key_padding_mask=side_padding_mask,
        )

        memory = torch.cat([enc_out, side_out], dim=1)
        memory_padding_mask = torch.cat([attention_mask == 0, side_padding_mask], dim=1)

        dec_out = self.decoder(
            tgt=decoder_state,
            memory=memory,
            tgt_mask=self._causal_mask(decoder_input_ids.size(1), decoder_input_ids.device),
            tgt_key_padding_mask=side_padding_mask,
