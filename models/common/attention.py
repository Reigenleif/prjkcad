...
class SDPAttention(nn.Module):
    ...
    def forward(
        ...
        if query.dim() == 4:
            B, T, A, D = query.shape
            query_2d = query.view(B * A, T, D)
            key_2d = key.view(B * A, key.size(1), D) if key.dim() == 4 else key.repeat_interleave(A, dim=0)
            value_2d = value.view(B * A, value.size(1), D) if value.dim() == 4 else value.repeat_interleave(A, dim=0)
            
            T_key = key_2d.size(1)
            q = self.q_proj(query_2d).view(B * A, T, self.n_heads, self.head_dim).transpose(1, 2)
            k = self.k_proj(key_2d).view(B * A, T_key, self.n_heads, self.head_dim).transpose(1, 2)
            v = self.v_proj(value_2d).view(B * A, T_key, self.n_heads, self.head_dim).transpose(1, 2)
            ...
            attn_out = attn_out.transpose(1, 2).contiguous().view(B * A, T, D)
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() == 4:
            B, T, A, D = x.shape
            x_2d = x.view(B * A, T, D)
            q = self.q_proj(x_2d).view(B * A, T, self.n_heads, self.head_dim).transpose(1, 2)
            k = self.k_proj(x_2d).view(B * A, T, self.n_heads, self.head_dim).transpose(1, 2)
